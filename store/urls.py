...
urlpatterns = [
    ...
    path('collections/<int:pk>/', views.collection_detail.as_view(), name='collection-detail'), # named URL pattern for HyperlinkedRelatedField in ProductSerializer. here pk is used instead of id because DRF uses pk by default for lookup fields.  name='collection-detail' is used to reference this URL pattern in serializers.py for HyperlinkedRelatedField.
]
